...
| 字段 | 类型 | 说明 | 依据 |
|---|---|---|---|
| `id` | string? | **机器身份，用户不填**：录入/首次加载瞬间系统生成**随机 UUID** 并**写回这段 JSON**（inline 回写 meta 表 + entry config 尽力、目录模式回写该文件）；写了就以用户写的为准（手写 JSON 者自负）。任意非空字符串都算合法；空串 / 非字符串 = 视为没有，重新生成并覆盖。**解析层兜底（仅回写失败时可达）用内容指纹** `t-` + 32hex，保证异常路径身份不漂 | 决策 25 / 30 |
| `title` | string | 用户可读名称：**任意文本（中文亦可）、随时可改**，**不参与身份** ⇒ 改名不改 `id`，历史不断链。缺省回退 code，再回退 id | 决策 25 / 30 |
