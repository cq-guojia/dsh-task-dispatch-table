...
| 17 | **Web 配置页 = 插件自带 client bundle，注册进 `plugins.bundle.config` slot（键 = 包名）——非 Config schema 自动渲染，也非 `plugins.item`** | 宿主源码核实（`/tmp/dsh-source`，0.1.6-alpha.2）：① 配置页不从 schema 自动生成，每个插件须自带浏览器半侧：manifest 声明 `"dsh": { "client": { "platform": "web", ... } }` + `exports['./client']` 指向 lazy-CJS factory 产物，宿主 modules 节点半侧扫描 Loader 行发现并供到 `/plugins/<包名>/client.js`，浏览器半侧挂在说明符 = 包名的那行 Loader 行上（packages/client/modules/src/index.ts:781-816）；② 产物格式 = `window.__ModuleLoader__.load({ id, factory(require){ ... return module.exports } })`（packages/client/tsdown.client.ts:618-624），externals（react 等）走基座模块表 PLATFORM_MODULES（packages/client/web/src/platform.ts:8-14），宿主预设未发布、第三方自行复刻构建（本仓库 = scripts/build-client.mjs）；③ `plugins.item` 被官方 ui-settings-plugins 的宿主平面配置页占用，组合包自己的配置页宿主契约明确指定 `plugins.bundle.config`（view: 'page'，渲染在组合包页面描述与行之间）或 `plugins.row.config`（ui-plugin-manager/src/client/slot-contract.ts:27-38、README.zh.md:46-58）；④ 读写走 `ctx.settingsScope.bind({ namespace })`（ui-settings/src/client/settings-scope.ts:232-298，写经 remote.settings.mutate 带 revision 设栅），命名空间被服务期间才注册页面（ui-settings-plugins/src/client/index.ts:124-148 同款 gating） |
