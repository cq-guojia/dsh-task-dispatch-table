...
>
> **最后更新**：2026-09-24 · **决策 28（面板内只读会话弹窗）已落码：数据链 = sessions.binding → 探测 open → uiConversation.target('chat') → legacy.nodes，冒烟 53 项全过，待真机验证**（见「六、下一步」第 3 条与 [`design/decisions.md`](design/decisions.md) 决策 28）。此前里程碑：2026-09-23 🎉 **首次全链路跑绿，端到端真机验收通过**——决策 24 落地后 `once7` / `once8` 两轮均以 `succeeded` 收官：pending → dispatched → agent 干活 → 调工具 `task_dispatch_table_receipt` → `receipt` → `receipt-pass`，回执在派发后 **4 秒 / 26 秒**到达，**全程无 `--db` 命令行、无沙箱报错、无权限申请**。决策 22（模型漏斗 + 会话归组）、23（部署默认 preset ⇒ 工具 / `AGENTS.md` 跟随系统）、24（per-agent 回执工具）**三项均已真机验证**。遗留待办（用户明确推迟）见「五、未决项」：U1 进程重启后的续跑 / 补跑、U2 失败即归档、U3 产出不验内容、U4 `logical_date` 是否显式注入
...
| 会话只读弹窗（决策 28） | 🔨 **已落码 + 冒烟 53 项全过，待真机验证**：面板内只读弹窗——**结构用官方**（`sessions.binding(id)` → 探测调 `session.open()` 拉尾页 → `uiConversation.binding(...).target('chat')`，渲染 `ChatSnapshot.legacy.nodes`；冷读 `session/follow`+`session/page` 退路**未启用**）、**画面自绘**（kind 映射：user/steering/assistant 气泡、tool-result/command 卡、turn-error/turn-max-tokens/model-retry 提示行；默认过滤 reasoning/context/unknown/compaction，未知 kind fallback 折叠 JSON）；「加载更早记录」走公开动词 `loadOlder()`；**只读、不可续聊**（用户已接受）。client 依赖已补**服务提供方**：`sessions` ← `dsh-api-session-controller`、`uiConversation` ← `dsh-client-ui-conversation`（修正：兄弟插件清单 `api-remotes`/`ui-session` 是它自己的 remote.* 消费面，不可照抄） |
