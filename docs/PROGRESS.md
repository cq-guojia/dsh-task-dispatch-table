...
| 2026-09-23 | **配置页临时调试面板落码（决策 16 例外）**：真机调试时容器内无 sqlite3、界面零反馈（旧版插件把 `contract.path` 必填的任务静默拒载是「没反应」根因）→ 配置页加「调试日志」弹窗。通道：host 侧 logger tee（warn/error 滚存 ≤20 条）+ `TaskStore.snapshot()`（全部实例 + 最近 40 条事件，detail 截 200 字符），组装 JSON 内容去重 + 2s 节流（会话事件逐条续租会写放大），经 owner `scope.update` 写本命名空间 `debugSnapshot` 字段（`packages/settings/.../index.ts:133,456,562`）→ 'settings/updated' 提交 → client `useSyncExternalStore` 订阅自动刷新；影子 ctx 两度真机崩溃后查 cordis 源码定论（vendor/cordis/src/reflect.ts）：① ctx 是 Proxy，set trap 拒绝一切赋值（:172-196，抛 "cannot set property without provide"）；② on/interval 等 mixin 方法不在 ctx 自有属性上，展开拷贝拿不到（:221，二崩 "logCtx.on is not a function"）→ **终版方案：ctx 一律原样传递，tee logger 作为显式参数传给 scheduler/reconciler/dispatch/tasks**（host.ts `HostLogger` 注释记录该事实，tasks 等模块签名相应改为收 logger）。tsc 双工程零报错；教训：mock 宿主是普通对象，测不出 cordis Proxy 语义，宿主侧 API 行为必须查源码后再写 |
