...
| 执行身份 + 刻度化调度（决策 25） | ✅ **落码 + 冒烟 39 项全过**：① 刻度化调度（小时级 / 分钟级 cron 一天可跑多条，旧实现一天只能一条）；② 执行主键 UUID + `UNIQUE(task_id, scheduled_at)` 防重（tick 幂等）；③ `task_defs` 定义身份登记（用户不写 id，系统按来源生成并记住）；④ **旧库兼容已实测**：用真机形态旧库（老 `task:日期` id + 各状态历史行）跑通迁移与 tick，历史行不丢、同一刻度不会被建出第二条；重复刻度优雅降级并告警、**不静默删数据** |
