...
>
> **最后更新**：2026-09-23 · 真机确认**决策 23 生效**（agent 有 fs/bash 工具、读到工作区 `Temp/AGENTS.md`、产物文件真建出来了），但回执被**沙箱**挡住——agent 的 bash 在 `workspace-write` 模式下只能写工作区，写不了宿主状态库；拍板**决策 24**（回执通道改为插件自带的 per-agent 工具 `task_dispatch_table_receipt`，并在提示词里写死「重试 ≤3 次后立即停手、禁止其他手段」）并落码，build 通过；待重装真机验证
...
| 回执通道（决策 24） | ✅ 落码 + build 通过：`setup(agentCtx)` 注册 per-agent 工具 `task_dispatch_table_receipt`（插件进程内写库、实例/会话身份闭包注入）；提示词写死「失败重试 ≤3 次 → 立即停手、禁止其他手段」；工具名 = 包名去 `dsh-` 前缀 + `_receipt`（`task_dispatch_table_receipt`，防宿主同层抛错 / 跨层遮蔽）；`status` 大小写与空白归一、**存声明值**；注册不上即判失败（`receipt-tool-unavailable`）；`submit.js` 降为手动备用；待重装真机验证 |
| 端到端联调 | 🟡 已跑通到「派发 → agent 用工具干活 → 产出文件」，卡在回执写库被沙箱拒绝（根因已定并于决策 24 修复）⇒ 待重装后重跑全链路 |
...
| [`design/decisions.md`](design/decisions.md) | 24 条已定型决策 + 理由（勿重复讨论）、决策 12 展开、命名查重记录 |
...
1. **真机重装后重跑全链路（观测入口 = 配置页调试面板）**：先 `npm run build` 再重装插件，换新 id 造一条几分钟后到点的 `once` 任务（建议加 `"retry": {"maxAttempts": 2}`，否则失败即终态、不重试）。到点看面板走完：pending → dispatched → **`dispatch` 事件（应带 `provider` / `model` / `modelSource` / `agentPreset`）** → agent 干活 → **调工具 `task_dispatch_table_receipt`** → `receipt` 事件 → `succeeded`。**本轮四个关键观测点**：① 工具目录里**有 `task_dispatch_table_receipt`**，且**用户自己新建的会话里没有**这张工具（per-agent 生效）；② agent 一次调用即成功，**没有**出现拷库 / `chmod` / 换 `sqlite3` 之类绕道动作；③ 故意给个非法 `status` 试一次，看它是否按提示「重试 ≤3 次后立即停手」；④ 不交回执时追问 ×2 → `failed`（`receipt-missing-after-nudge`）仍照旧。**once 改期语义（已拍板）**：实例身份 = task_id + once 日期 ⇒ **同日改时刻无效**（建行幂等跳过 scheduler.ts:68 + 重排只重算 pending attempt=0，unknown/终态冻结）；改到明日则 id 不变即可、到点自动补建；**今日验证须换新 id**。另：崩溃排查隔离的文件（宿主数据根 quarantine/ 下）待确认后清理
