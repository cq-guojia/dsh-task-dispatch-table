...
| 执行身份 + 刻度化调度（决策 25） | ✅ **落码 + 冒烟 28 项全过**：① 刻度化调度（小时级 / 分钟级 cron 一天可跑多条，旧实现一天只能一条）；② 执行主键 UUID + `UNIQUE(task_id, scheduled_at)` 防重（tick 幂等）；③ `task_defs` 定义身份登记（用户不写 id，系统按来源生成并记住）；④ 旧库加索引即升级、历史行不丢 |
