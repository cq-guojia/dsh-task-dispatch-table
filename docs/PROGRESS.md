...
> **这是什么**：本仓库**唯一**的进度真源，替代过去的「工作交接表」。
> **怎么用**：换设备/换会话后，**只读这一份**即可继续推进。
> **怎么维护**：**就地更新本文件**——文件名和位置固定，不新建、不改名、不搬走。
> 每次推进后追加到「七、进展日志」，并同步更新「五、未决项」和「六、下一步」。
> ⚠️ README / Issues / 聊天记录都不算真源。
>
> **本文件范围**：只记**开发项**（设计 → 数据模型 → 代码 → 发布）。
> 内容一旦**定型**就升格到 [`docs/design/`](design/) 下的专题文档，这里只留链接。
>
> **最后更新**：2026-09-21 · 文档重组（原根目录 `092945-跟踪-…md` 拆分而来）
...
| [`design/decisions.md`](design/decisions.md) | 13 条已定型决策 + 理由（勿重复讨论）、命名查重记录 |
| [`design/task-manual-vs-skill.md`](design/task-manual-vs-skill.md) | 为什么任务手册是 MD 文件而不是 skill |
...
1. **本文件是唯一的进度真源**，就地更新；定型内容升格到 `docs/design/` 后，本文件只留链接，避免双份副本过期。
2. **仓库布局**：用户文档在根目录（`README.md` / `LICENSE`），设计文档在 `docs/design/`，进度在 `docs/PROGRESS.md`。不使用按日期流水归档的目录约定——版本历史由 Git 承担。
3. **文件命名**：固定名、纯 ASCII 小写 kebab-case，**表角色不表时间**（不带日期前缀、不带版本号）。
4. **写操作闸门**：涉及 destructive 操作（重写历史、强推、删远端）必须先获明确指令。
5. **先结论后证据**：结论必须可追溯到源码 `文件:行号` 或日志原文。
6. **少折腾**：先问「能不能直连 / 有没有更短路径」，再考虑加组件。
