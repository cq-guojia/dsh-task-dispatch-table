...
const instance = db
    ...
    .get(taskId, logicalDate);
if (instance === undefined)
    fail(`实例不存在: ${taskId} @ ${logicalDate}`);
...
db.prepare('INSERT INTO task_events (instance_id, ts, kind, detail) VALUES (?, ?, ?, ?)').run(instanceId, new Date().toISOString(), 'receipt', JSON.stringify({ status, outputs, note, session_id: sessionId }));
db.close();
console.log(`回执已提交: ${instanceId} (status=${status}${outputs.length > 0 ? `, outputs=${outputs.length} 项` : ''})`);
