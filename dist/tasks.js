 ...
 */
export function existingUuidIds(raw) {
    let data;
    try {
 ...
 */
export function ensureIdsInInlineJson(raw, existingIds) {
    const text = raw.trim();
    if (text === '')
 ...
 */
export function applyIdentity(def) {
    if (!isUuid(def.id))
        return null;
 ...
 */
export function firstSlotOnDay(task, day) {
