...
const RECEIPT_ACCEPTING = ['dispatched', 'running', 'unknown'];
 ...
 */
const foldFullWidth = (value) => value
    ...
    return {
        ...
        execute: async (args) => {
            try {
                ...
                if (!RECEIPT_ACCEPTING.includes(instance.status)) {
                    return {
                        ok: false,
 ...
 */
export function registerReceiptTool(agentCtx, deps) {
    const tools = agentCtx?.tools;
    if (tools === undefined || typeof tools.register !== 'function') {
        deps.logger.warn(`agent 作用域未暴露 tools 服务，${RECEIPT_TOOL_NAME} 未注册——本会话无法提交回执（决策 24）`);
        return false;
    }
    try {
