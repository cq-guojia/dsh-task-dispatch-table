...
export function checkContract(task, workspacePath, dispatchedAtMs) {
    const contractPath = resolve(workspacePath, task.contract.path);
    if (!existsSync(contractPath))
        return { ok: false, reason: 'contract-missing', detail: contractPath };
...
export function createReconciler({ ctx, store, options }) {
    ...
    function finishTerminal(instance, status, reason, detail) {
        ...
        if (detail !== undefined)
            store.appendEvent(instance.id, 'contract_check', { reason, detail });
    ...
    function settleByContract(instance) {
        const task = taskOf(instance);
        if (task === undefined) {
