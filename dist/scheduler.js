import { durationMs, firstSlotOnDay, loadTasks, logicalDateOf, onceScheduledAt, parseInlineTasks, scheduledSlotsFor, } from './tasks.js';
...
export function createScheduler({ ctx, logger, store, reconciler, config }) {
     ...
     */
    function ensureInstances(currentTasks, tickMs) {
        const now = Date.now();
        for (const task of currentTasks) {
            ...
            if (slots.length > MAX_ENSURE_SLOTS)
                slots = slots.slice(-MAX_ENSURE_SLOTS);
    ...
    function reschedulePass(currentTasks) {
        for (const task of currentTasks) {
            ...
            for (const instance of pendings) {
                ...
                if (slots.includes(instance.scheduled_at))
                    continue; // 仍是有效刻度：无需重排
